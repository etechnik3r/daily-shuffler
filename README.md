...
- **Visuelles Feedback:** Der Timer ändert seine Farbe bei nahendem Zeitende (Gelb &rarr; Orange &rarr; Rot) und zeigt bei Überziehung einen Overtime-Indikator an – mit für jede Ansicht passenden Symbolen neben der Uhr (z. B. abgefahrener Zug bei der Abfahrtstafel).
...
- **Sounds (optional):** Jede Ansicht hat ihren eigenen Klang – das Ratschen des Glücksrads, der Hebel und die Walze der Slotmaschine, die Karte, die auf den Stapel klatscht, das Klappern der Abfahrtstafel, der Bahnhofsgong des Linienplans, das Sirren des E-Motors und das leise Rieseln der Sanduhr. Dazu ein Warnton, wenn die letzten zehn Sekunden anbrechen, und eine Fanfare zum Schluss. **Standardmäßig aus** und in den Einstellungen einschaltbar. Alle Klänge werden im Browser erzeugt (Web Audio) – es werden keine Audiodateien geladen, die App bleibt eine einzelne Datei und offlinefähig.
...
- **Abfahrtstafel-Ansicht:** Eine Fallblatt-Anzeige wie am Bahnhof. Alles auf dem Brett ist eine Klappe – die Position, der Name, die Statusspalte und die Spaltenüberschriften. Pro Klick auf „Anzeige stellen" (oder Leertaste) klappert eine Zeile durch zufällige Zeichen und rastet auf dem nächsten Sprecher ein. Anders als bei der Slotmaschine bleibt die komplette Reihenfolge sichtbar: erledigte Zeilen sind abgedunkelt, die übrigen warten als leere Klappen. Die Klappen füllen die Zeile über die ganze Breite, nicht nur bis zum Namensende, und die Tafel hat von Anfang an ihre volle Höhe – sie wächst beim Aufdecken nicht nach. Gelb bedeutet: diese Person ist jetzt dran – während des Klapperns bleibt die Zeile grau, das Einrasten *ist* der Farbwechsel. Zieht die Person über ihre Redezeit, schlägt die ganze Zeile von Gelb auf **Rot** um und die Statusspalte wechselt von „jetzt“ auf **„verspätet“**.
...
- **Sounds:** Schaltet die Geräusche zu den Animationen an. Ab Werk **aus** – auf einem geteilten Bildschirm soll nichts unerwartet losklingeln. Die Einstellung greift sofort (ein kurzer Bestätigungston bestätigt das Einschalten) und wird lokal gespeichert.
...
- **[PROJECT.md](PROJECT.md):** Idee, Architektur und Dateistruktur des Projekts – der schnelle Einstieg, um den Taktgeber weiterzuentwickeln.
- **[CHANGELOG.md](CHANGELOG.md):** Versionshistorie. Die Version entspricht der `CACHE_VERSION` im Service Worker `sw.js`.
